...
@section('content')
    <div class="content-wrapper px-4">
        <div class="add-expert-wrapper">
            <h4><i class="fa fa-gears"></i> Edit Expert</h4><hr>
            @if(session()->has('error'))
                {!! '<div class="alert alert-danger" >'.session("error").'</div>' !!}
            @elseif(session()->has('success'))
                {!! '<div class="alert alert-success" >'.session("success").'</div>' !!}
            @endif
            <form action="{{ route('admin.expert.update', $expert->id) }}" method="post" >
                @csrf
                <div class="row mb-5" >
                    <div class="col-md-3" >
                        <div id="p-image-cover" >
                            <div id="p-image" >
                                <img src="{{ asset("storage/profile_img/{$expert->expert->profile_picture}") }}" height="100%" >
                            </div>
                    ...
                    <div class="col-md-9" >
                        <div class="form-row">
                            <div class="form-group col-md-8">
                                ...
                                <input type="Email" class="form-control" name="email" id="email" placeholder="E.g expert@gmail.com" value="{{ $expert->email }}" disabled readonly >
                                <div class="invalid-feedback text-danger d-block" role="alert">
                                    <p><strong>Cannot be changed</strong></p>
                                </div>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="">Telephone</label>
                                <input type="tel" class="form-control" name="tel" id="" value="{{ $expert->expert->telephone }}" >
                                @if ($errors->has('tel'))
                                    <div class="invalid-feedback text-danger d-block" role="alert">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="expertise" id="" placeholder="e.g Medical doctor" value="{{ $expert->expert->expertise }}" >
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" min="0" class="form-control" name="experience" id="" value="{{ $expert->expert->experience }}" >
                ...
                <div class="form-group">
                    ...
                    <textarea name="about" class="form-control" id="" rows="10">{{ $expert->expert->about }}</textarea>
                    @if ($errors->has('about'))
                        <div class="invalid-feedback text-danger d-block" role="alert">
...
@section('scripts')
    <script>
        ...
        let currentProfileImage = '{{ asset("storage/profile_img/{$expert->expert->profile_picture}") }}'
        ...
        $("form").submit(function (e) {
            ...
            html2canvas(document.querySelector("#p-image")).then(canvas => {
                ...
                if(currentProfileImage !== $("#p-image img").attr('src')){
